...
const setActiveHours = (client: PrismaClient): RequestHandler =>
async (req:RequestWithJWTBody, res) => {
    const {startSunday,endSunday,startMonday,endMonday,startTuesday,endTuesday,
        startWednesday,endWednesday,startThursday,endThursday,startFriday,endFriday,
    ...
    try {
        ...
        console.log(error)
        res.status(400)
        res.json({user:null, token:null, error:"Could not update Active Hours."})
    }
}
